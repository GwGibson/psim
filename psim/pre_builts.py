...
# TODO: clean this up
def kinked_meyer(angle: float, l: int, spec: float, t_high: float,
                 t_low: float, t_init: float, t_eq: float) -> ModelBuilder:
    
    #gets the measurement points along the phonon path 
    def get_phonon_path(x_dist, kink_length, r_angle):
                
        midpoints = []
        #first 3 straight points
        lc = bx - x_dist/3
        kd = kink_length/3
        x1 = lc/2
        x2 = x1 + lc
        x3 = x2 + lc
        midpoints.extend([x1, x2, x3])
        #first transition area
        if (angle > 0 and angle < 90):
            x_start = x3 + lc/2
            if (angle > 30):
                x4 = x_start + 3*by/4*np.tan(r_angle/2)/2
                x5 = x4 + 3*by/4*np.tan(r_angle/2)
                midpoints.extend([x4, x5])
            else:
                x5 = x_start + 3*by/4*np.tan(r_angle/2)
                midpoints.extend([x5])
        else:
            x5 = bx*3 
        #first kink
        if (angle > 30):
            x_start = x5 + 3*by/4*np.tan(r_angle/2)/2
        else:
            x_start = x5 + 3*by/4*np.tan(r_angle/2)
        x6 = x_start + kd/2
        x7 = x6 + kd
        x8 = x7 + kd
        midpoints.extend([x6, x7, x8])
        #top transition area
        if (angle > 0 and angle < 90):
            x_start = x8 + kd/2
            if (angle > 30):
                x9 = x_start + by/2*np.tan(r_angle)/2
                x10 = x9 + by/2*np.tan(r_angle) 
                midpoints.extend([x9, x10])
            else:    
                x10 = x_start + by/2*np.tan(r_angle)
                midpoints.extend([x10])
        else:
            x10 = x8+kd*np.cos(r_angle)/2
        #second kink
        if (angle > 30):
            x_start = x10 + by/2*np.tan(r_angle)/2
        else:
            x_start = x10 + by/2*np.tan(r_angle)
        x11 = x_start + kd/2
        x12 = x11 + kd
        x13 = x12 + kd
        midpoints.extend([x11, x12, x13])
        #second transition area
        if (angle > 0 and angle < 90):
            x_start = x13 + kd/2
            if (angle > 30):
                x14 = x_start + 3*by/4*np.tan(r_angle/2)/2
                x15 = x14 + 3*by/4*np.tan(r_angle/2)
                midpoints.extend([x14, x15])
            else:
                x15 = x_start + 3*by/4*np.tan(r_angle/2)
                midpoints.extend([x15])
        else:
            x15 = bx*15 
        #last 3 staight segments
        if (angle > 30):
            x_start = x15 + 3*by/4*np.tan(r_angle/2)/2
        else:    
            x_start = x15 + 3*by/4*np.tan(r_angle/2)
        x16 = x_start + lc/2
        x17 = x16 + lc
        x18 = x17 + lc
        midpoints.extend([x16, x17, x18])
        return midpoints
    ...
    top_tri_length = by * np.tan(r_angle) 
    kink_length = 2*l - by/2*np.tan(r_angle) - by/2*np.tan(r_angle/2)
    top_y = by/2 + np.sin(r_angle) * 2*l - by/2/np.cos(r_angle)
    if (top_tri_length/2 - by/2*np.tan(r_angle/2) > 2*l):
    ...
    x_dist = 2*bx * np.tan(np.radians(angle/2))
    midpoints = get_phonon_path(x_dist, kink_length, r_angle)
    # print(f'{angle}:', end=': ')
    # print(midpoints)
    
    # Build first straight segment
    for _ in range(3):
        s_id = b.addSensor(material_name, t_init)
        b.addRectangularCell((x, y), (x+bx-x_dist/3, by), s_id, spec)
        x += bx-x_dist/3
    ...
    if (angle == 90):
        ...
        for _ in range(4):
            s_id = b.addSensor(material_name, t_init)
            b.addRectangularCell((orig_x, y), (orig_x+bx, by), s_id, spec)
            orig_x += bx

        orig_x = x
        for _ in range(4):
            s_id = b.addSensor(material_name, t_init)
            b.addRectangularCell((orig_x, y+by), (orig_x+bx, by+by), s_id, spec)
            orig_x += bx

        orig_x = x
        for _ in range(4):
            s_id = b.addSensor(material_name, t_init)
            b.addRectangularCell(
                (orig_x, y+by+by), (orig_x+bx, by+by+by), s_id, spec)
            orig_x += bx
        ...
        if (angle > 0 and angle < 360):
            new_x = x+x_dist
            s_id = b.addSensor(material_name, t_init)
            b.addTriangleCell((x, 0), (x, by), (new_x, 0), s_id, spec)
            x = new_x
            
            new_x = x + np.cos(r_angle)*x_dist
            new_y = np.sin(r_angle)*x_dist
            if (angle > 30):
                s_id = b.addSensor(material_name, t_init)
            b.addTriangleCell((new_x, new_y),
                              (x-x_dist, by), (x, 0), s_id, spec)
            x = new_x
            y = new_y

        # Build first kink segment
        inc_dist = kink_length/3
        for _ in range(3):
            s_id = b.addSensor(material_name, t_init)
            b.addQuadrilateralCell(
                (x, y), inc_dist, by, angle, s_id, spec)
            x += np.cos(r_angle)*inc_dist
            y += np.sin(r_angle)*inc_dist

        # Build top joining segment
        if (angle > 0 and angle < 360):
            x = l - x_dist/2 + np.cos(r_angle) * 2*l
            y = top_y
            s_id = b.addSensor(material_name, t_init)
            b.addTriangleCell((x, y), (x, y + by/np.cos(r_angle)), (x-np.sin(r_angle)*by,
                                                                    y+np.cos(r_angle)*by), s_id, spec)
            if (angle > 30):
                s_id = b.addSensor(material_name, t_init)
            b.addTriangleCell((x, y), (x, y + by/np.cos(r_angle)),
                              (x+np.sin(r_angle)*by, y+np.cos(r_angle)*by), s_id, spec)
        ...
        for _ in range(3):
            s_id = b.addSensor(material_name, t_init)
            b.addQuadrilateralCell(
                (x, y), inc_dist, -by, 360-angle, s_id, spec)
            x += np.cos(r_angle)*inc_dist
            y -= np.sin(r_angle)*inc_dist
        ...
        if (angle > 0 and angle < 360):
            s_id = b.addSensor(material_name, t_init)
            b.addTriangleCell((x-by*np.sin(r_angle), np.sin(r_angle)*x_dist),
                              (x, y), (x-x_dist, 0), s_id, spec)
            if (angle > 30):
                s_id = b.addSensor(material_name, t_init)
            b.addTriangleCell((x, 0),
                              (x, y), (x-x_dist, 0), s_id, spec)

    # Build second straight segment
    for _ in range(3):
        s_id = b.addSensor(material_name, t_init)
        b.addRectangularCell((x, 0), (x+bx-x_dist/3, by), s_id, spec)
        x += bx-x_dist/3
    # print(x)

    b.addSurface((0., 0.), (0., by), t_high)
    b.addSurface((x, 0), (x, by), t_low)
...
def layered_kinked(angle: float, l: int, spec: float, t_high: float,
    ...
    if (angle < 0 or angle > 90):
        raise ValueError('Angle must be in the range [0,90].')
    # if (num_rows < 2):
    #     b, midpoints = kinked_meyer(angle, l, spec, t_high, t_low, t_init, t_eq)
    #     return b
